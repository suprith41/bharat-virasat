  ...
  return (
    <div className="hidden lg:flex relative overflow-hidden items-center justify-center">
      <div
        ...
        style={{ background: "linear-gradient(135deg, oklch(0.95 0.05 70) 0%, oklch(0.88 0.1 30) 50%, oklch(0.78 0.13 350) 100%)" }}
      />
      <div className="absolute inset-0 paisley-pattern opacity-50" />

      {/* Mandala */}
      <svg viewBox="0 0 400 400" className="relative w-[80%] max-w-md chakra-spin opacity-90" style={{ animationDuration: "40s" }}>
        <defs>
        ...
        <g fill="none" stroke="url(#mandala-grad)" strokeWidth="1.5">
          ...
          {Array.from({ length: 24 }).map((_, i) => {
            ...
            return <path key={i} d={`M${x1},${y1} Q${(x1+x2)/2 + Math.cos(a + Math.PI/2)*8},${(y1+y2)/2 + Math.sin(a + Math.PI/2)*8} ${x2},${y2}`} />;
          })}
        </g>
      ...
      {[0, 1, 2, 3, 4].map((i) => (
        <div
          ...
          style={{
            left: `${15 + i * 18}%`,
            top: `${20 + (i % 2) * 60}%`,
            background: "radial-gradient(circle, oklch(0.95 0.15 75) 0%, oklch(0.7 0.2 40 / 0) 70%)",

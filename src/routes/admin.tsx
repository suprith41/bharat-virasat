...
import {
  LayoutDashboard, MapPinned, Sparkles, Users, BarChart3, Settings,
  TrendingUp, TrendingDown, Plus, Search, Edit2, Trash2, MoreVertical, ArrowUp,
} from "lucide-react";
import {
...
const KPIS = [
  { label: "Total States", value: "28", delta: "+2", up: true, color: "var(--saffron)", spark: [12, 14, 13, 18, 22, 25, 28] },
  { label: "Active Users", value: "12.4k", delta: "+18%", up: true, color: "var(--peacock)", spark: [8, 9, 11, 10, 12, 13, 12.4] },
  { label: "Content Items", value: "847", delta: "+34", up: true, color: "var(--lotus)", spark: [600, 650, 700, 720, 780, 820, 847] },
  { label: "Reports", value: "3", delta: "-2", up: false, color: "var(--leaf)", spark: [8, 7, 6, 5, 4, 5, 3] },
];
      ...
      <aside className="w-60 shrink-0 glass-dark border-r border-white/10 flex flex-col">
        ...
        <div className="p-4 border-t border-white/10">
          <div className="glass-dark rounded-xl p-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full saffron-gradient flex items-center justify-center text-white font-bold">A</div>
        ...
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1">Manage Bharat Virasat's living archive.</p>
            </div>
            <div className="flex gap-3">
              <div className="glass-dark rounded-full flex items-center px-4 py-2 gap-2 w-72">
                <Search className="h-4 w-4 text-muted-foreground" />
                <input placeholder="Search…" className="flex-1 bg-transparent focus:outline-none text-sm" />
              </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {KPIS.map((k) => (
          <div key={k.label} className="glass-dark rounded-2xl p-5 relative overflow-hidden">
            <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-30 blur-2xl" style={{ background: k.color }} />
            <div className="relative">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">{k.label}</div>
              <div className="font-display text-3xl font-bold mt-2">{k.value}</div>
              <div className={`mt-2 inline-flex items-center gap-1 text-xs font-semibold ${k.up ? "text-leaf" : "text-destructive"}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="h-72">
            <ResponsiveContainer>
              <LineChart data={VISITS}>
                ...
                <Tooltip contentStyle={{ background: "oklch(0.24 0.04 250)", border: "1px solid oklch(1 0 0 / 0.1)", borderRadius: 12 }} />
                <Line type="monotone" dataKey="visits" stroke="oklch(0.78 0.17 60)" strokeWidth={3} dot={{ fill: "oklch(0.78 0.17 60)", r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="glass-dark rounded-2xl p-6">
          ...
          <div className="h-56">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={STATE_PIE} dataKey="value" innerRadius={45} outerRadius={75} paddingAngle={3}>
                  {STATE_PIE.map((s) => <Cell key={s.name} fill={s.color} />)}
                </Pie>
                <Tooltip contentStyle={{ background: "oklch(0.24 0.04 250)", border: "1px solid oklch(1 0 0 / 0.1)", borderRadius: 12 }} />
              </PieChart>
            </ResponsiveContainer>
        ...
        <thead className="text-xs uppercase tracking-wider text-muted-foreground border-b border-white/10">
          <tr>
            <th className="text-left px-6 py-3 font-semibold">
              <div className="inline-flex items-center gap-1 cursor-pointer">State <ArrowUp className="h-3 w-3" /></div>
            </th>
        ...
        <tbody>
          {STATE_LIST.map((s) => (
            <tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
              ...
              <td className="px-6 py-4">{s.festivals.length + s.food.length + s.art.length + s.traditions.length + s.heritage.length}</td>
              <td className="px-6 py-4">{s.unescoSites}</td>
              <td className="px-6 py-4 text-right">
                <div className="inline-flex items-center gap-1">
                  <button className="p-2 rounded-lg hover:bg-white/10"><Edit2 className="h-3.5 w-3.5" /></button>
                  <button className="p-2 rounded-lg hover:bg-white/10 text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                  <button className="p-2 rounded-lg hover:bg-white/10"><MoreVertical className="h-3.5 w-3.5" /></button>
                </div>
              </td>
            </tr>
          ))}
    ...
    <div className="grid lg:grid-cols-2 gap-6 animate-fade-up">
      <div className="glass-dark rounded-2xl p-6">
        ...
        <form className="space-y-4 text-sm" onSubmit={(e) => e.preventDefault()}>
          <div>
            ...
            <input className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-saffron" placeholder="e.g. Pushkar Camel Fair" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">State</label>
              <select className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 focus:outline-none focus:border-saffron">
                {STATE_LIST.map((s) => <option key={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Category</label>
              <select className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 focus:outline-none focus:border-saffron">
                <option>Festival</option><option>Food</option><option>Art</option><option>Tradition</option><option>Heritage</option>
              </select>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Description</label>
            <textarea rows={5} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-saffron resize-none" placeholder="Tell the story…" />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Image</label>
          ...
          <button className="w-full py-3 rounded-full text-sm font-semibold text-white saffron-gradient">Save entry</button>
        </form>
      </div>
      ...
      <div className="glass-dark rounded-2xl p-6">
        ...
        <div className="space-y-3">
          {STATE_LIST.flatMap((s) => s.festivals.slice(0, 1).map((f) => ({ ...f, state: s.name, gradient: s.bannerGradient }))).map((f, i) => (
...
function UsersView() {
  const users = [
    ...
    { name: "Meera Banerjee", email: "meera@example.com", joined: "Jan 2026", state: "West Bengal" },
      ...
      <div className="space-y-2">
        {users.map((u) => (
          <div key={u.email} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white/5">
            <div className="w-10 h-10 rounded-full saffron-gradient flex items-center justify-center text-white font-bold text-sm">{u.name[0]}</div>
  ...
  return (
    <div className="glass-dark rounded-2xl p-8 animate-fade-up max-w-2xl">
      ...
      <div className="space-y-5 text-sm">
        {["Public registration", "Email notifications", "Comment moderation", "Allow user submissions"].map((s) => (
